...
# example tuning curves
cell_IDs = ["X087_20210730_159",  # FOF graded
            "X087_20210730_320",    # ADS graded
            "X087_20210730_237", # FOF categorical
            "X087_20210730_375" # ADS categorical
            ]
...
p['t0_ms'] = 150
p['t1_ms'] = 500
savepath = SPEC.RESULTDIR + "hanks_tuning_curves_2/"

fig, axs = plt.subplots(2,2, figsize = (4,4), sharex=True, sharey = True)


for ax_cellnum, cell in enumerate(cell_IDs):
    ...
    axs.ravel()[ax_cellnum].scatter(xc, mean_tun, s = 10, color = colors, zorder = 3)
    axs.ravel()[ax_cellnum].errorbar(xc, mean_tun, 
                      yerr= std_tun, 
                      ls = "none", 
                      c = 'k', 
                      lw = 0.5, 
                      zorder = 1)
    axs.ravel()[ax_cellnum].plot(xc[1:-1], sigmoid_4(xc[1:-1], *popt), c = 'k', ls = '-', lw = 0.5, zorder = 2)
    axs.ravel()[ax_cellnum].tick_params(axis='both', which='major', labelsize=8)
    axs.ravel()[ax_cellnum].tick_params(axis='both', which='minor', labelsize=8)
    axs.ravel()[ax_cellnum].set_title(cell + ' Slope = '  + str(np.round(popt[1]*popt[3]/4,2)), fontsize = 8)

axs[1,0].set_xlabel('Accumulator value')
axs[1,1].set_xlabel('Accumulator value')
axs[0,0].set_ylabel('Fractional change in \nfiring rate $\it{r}$')
axs[1,0].set_ylabel('Fractional change in \nfiring rate $\it{r}$')
plt.tight_layout()
sns.despine()
savethisfig(SPEC.FIGUREDIR + "figure2/",'example_tuning_curves')


# produce legend
fig = plt.figure(figsize = (3,1))
axs = fig.gca()
axs.scatter(xc, [1]*len(xc), s = 10, color = colors)
axs.plot(xc[1:-1], [1]*(len(xc)-2), lw = 0.5, c = 'k', zorder = 0)
axs.set_xticks(np.round(xc)[::2], np.round(xc).astype(int)[::2])
axs.get_yaxis().set_visible(False)
plt.tight_layout()
sns.despine()
savethisfig(SPEC.FIGUREDIR + "figure2/",'example_tuning_curves_legend')
